...
import { formatINRFull } from '../data/constants'

export default function ProjectPickerScreen({ onPick }) {
  const { user, projects } = useData()

  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <TopBar title="Choose project"/>
      <div style={{ padding: '12px 16px 4px' }}>
        <p style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', margin: 0 }}>
          You're assigned to {projects.length} projects. Pick one to start logging.
        </p>
      </div>
